...
calculate.moving.avg <- function(data, device_type, site_col, sensor_col) {
  require(zoo)
  require(circular)
  ...
  if (device_type == "staroddi") {
    ...
    data <- lapply(data, function(df) {
      df %>%
        mutate(
          temp_ma01 = zoo::rollmean(Temp, k = 3, fill = NA),
    ...
    # ma03 = Calculating a rolling mean over 180 points, means over 1 min * 180 = 3 hours.
    # ma04 = Calculating a rolling mean over 360 points, means over 1 min * 360 = over around 360 minutes = 6 hours.
    data <- split(data, list(data[[site_col]], data[[sensor_col]]), drop = TRUE)
    data <- lapply(data, function(df) {
      df %>%
        mutate(
          temp_ma01 = zoo::rollmean(Temperature, k = 30, fill = NA),
    ...
    stop(
      "Invalid device type. Choose 'staroddi', 'tcm-current', 'tcm-temperature', or 'aquameasure'."
    )
  }
