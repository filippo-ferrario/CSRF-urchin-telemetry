# ===============================================================================
# Name   	: Georeference Cameras for BIC-PIL
# Author 	: Filippo Ferrario
# Date   	: 03-30-2023 [dd-mm-yyyy]
# Version	: 
# URL		: 
# Aim 		:
# ===============================================================================
...
pics<-read.csv('output/datasets/photogrammetry/BIC-PIL-paired_synced.csv', na.strings = "")
ref_pics<-read.csv('data/photogrammetry/BIC-PIL-pic2georef-main_camera.csv', na.strings = "") %>% as_tibble()
...
pil_gps<-gps %>%	
		filter(site=='BIC-PIL', receiver_id!='Ref') %>%
...
# Y=0
t030_0<-destPoint(p=st_coordinates(pil_gps[pil_gps$gcp=='T0-0',]), b=hc0[1], d=dists) %>%
		as.data.frame(.) %>%
...
t3060_0<-destPoint(p=st_coordinates(pil_gps[pil_gps$gcp=='T30-0',]), b=hc0[2], d=dists) %>%
		as.data.frame(.) %>%
...
#  Y=30
t030_30<-destPoint(p=st_coordinates(pil_gps[pil_gps$gcp=='T0-30',]), b=hc30[1], d=dists) %>%
		as.data.frame(.) %>%
...
t3060_30<-destPoint(p=st_coordinates(pil_gps[pil_gps$gcp=='T30-30',]), b=hc30[2], d=dists) %>%
		as.data.frame(.) %>%
...
# Y=60
dists030_60<-seq(6, as.numeric(st_distance(pil_gps[pil_gps$gcp=='T0-60',],pil_gps[pil_gps$gcp=='T30-60',]))-6, length.out=4)
t030_60<-destPoint(p=st_coordinates(pil_gps[pil_gps$gcp=='T0-60',]), b=hc60[1], d=dists030_60) %>%
		as.data.frame(.) %>%
...
# dists3060_60<-seq(3, as.numeric(st_distance(pil_gps[pil_gps$gcp=='h',],pil_gps[pil_gps$gcp=='i',]))-3, length.out=9)
t3060_60<-destPoint(p=st_coordinates(pil_gps[pil_gps$gcp=='T30-60',]), b=hc60[2], d=dists) %>%
		as.data.frame(.) %>%
...
script_ver<-'imagery_georef-BIC-PIL.R'
...
shaRe::export_dictionary(description=txt, varTable= geo_dict, path='output/datasets/photogrammetry/georeferenced_images-BIC-PIL-dictionary.txt')

write.csv(geo_pics,file='output/datasets/photogrammetry/georeferenced_images-BIC-PIL.csv', quote=F, row.names=F)
