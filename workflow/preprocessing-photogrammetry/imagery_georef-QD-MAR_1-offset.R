# ===============================================================================
# Name   	: Georeference Cameras for Marina 1
...
ref_pics<-read.csv('data/photogrammetry/QD-MAR_1-pic2georef-main_camera.csv', na.strings = "")
depths<-read.csv('output/datasets/photogrammetry/BC-targets_depth.csv', na.strings = "")
pics<-read.csv('output/datasets/photogrammetry/QD-MAR_1-paired_synced.csv', na.strings = "")
...
# Correct labels of sequence in shooting group 20230604-T28_00Rx-T60_00Lx to get rid of the "_" before the side tag. 
# This is causing error later in the creation of offset

pics$sequence<-sub(pics$sequence, pattern='_L', replacement='L')
pics$sequence<-sub(pics$sequence, pattern='_R', replacement='R')
pics$sequence

ref_pics$sequence<-sub(ref_pics$sequence, pattern='_L', replacement='L')
ref_pics$sequence<-sub(ref_pics$sequence, pattern='_R', replacement='R')
ref_pics$sequence

ref_pics$sequence_georef<-sub(ref_pics$sequence_georef, pattern='_L', replacement='L')
ref_pics$sequence_georef<-sub(ref_pics$sequence_georef, pattern='_R', replacement='R')
ref_pics$sequence_georef
...
# split ref dataset in a list by leg (2023-03-22 note: added shooting group as factor because there were pics of the same leg taken in 2 different dives! )
...
# ==================================================================================================================================================
# 3) break each leg line into the correct number of steps (i.e., points along the line) for that leg. This will define a gps track.
# ==================================================================================================================================================
...
picsList<-filter(pics, !is.na(leg)) %>% split(., f=paste(.$shooting_group,.$leg))
...
picsList_coords<-lapply(names(picsList), function(x){
			...
			legx<-unlist(strsplit(x,split=' '))[2]

			# Which transect in legs is in the name of the list element?  
			trs_id<-sapply(legs$trs, function(y) grepl(legx,pattern=y))  
			# Which leg in legs is in the name of the list element?
			if (!reverse) {
				leg_id<-sapply(legs$leg, function(y) grepl(legx,pattern=y))
				} else { 
				leg_id<-sapply(legs$rev_leg, function(y) grepl(legx,pattern=y))
				}
...
# calculate offset for support cameras 
# -----------------------------------------------------------------

# calculate the heading perpendicular to the transect lines, both for directions 0-60 and 60-0
xlegs_hdg<-legs %>%
			st_geometry(.) %>%
			lapply(function(x){
				# browser()
				bearing0_60<-st_line_sample(x,sample=c(0,1)) %>% 
					st_cast('POINT') %>%
					st_coordinates(.) %>% 
					bearing(p1=.[1,], p2=.[2,]) 
				crossLX<-bearing0_60-90
				crossRX<-bearing0_60+90
				data.frame(crLX=crossLX, crRX=crossRX)
				}) %>%
			bind_rows()%>%
			bind_cols(legs,.)

# as.data.frame(xlegs_hdg)

# format picsList_coords_df in  long format; and calculate offset from inner camera depending on direction. 
head(picsList_coords_df)
head(ref_pics)

# # check 
# unique(ref_pics$sequence_georef)%in%unique(picsList_coords_df$leg)
# 

# create a list with files in each leg, formatted long
leg_file_list<-picsList_coords_df %>%
				pivot_longer(cols=matches('\\.'),names_to='cameraID', values_to='file') %>%
				filter(file!="NA") %>% 
				left_join(ref_pics, by=c('file'='file_name','leg'='sequence_georef', 'shooting_group', 'sequence')) %>% 
				split(f=paste(.$shooting_group,.$leg)) 
 
# unique(leg_file_list$sequence)


xy_adj<-lapply(names(leg_file_list), function(x){
		# browser()
		tmp<-leg_file_list[x][[1]]
		tmp$offset<-NA
		# define of offsets per cameras depending on inner camera identity
		inner<- tmp %>%
				filter(camera_role=='inner_camera') 
		inner<-unique(inner$cameraID.x)

		# Accomodate the fact that one shooting group had 3 cameras while the other had 4
		# shooting group 20230604-T00_00Rx-T32_00Lx
		if(grepl(x, pattern='20230604-T00_00Rx-T32_00Lx') & grepl(inner, pattern='lx')) {
			tmp[grepl(tmp$cameraID.x, pattern='lx'),]$offset<-0.1     # consider the distance of the inner camera from the transect (Diver was swimming at about 1m from the transect, and the external cameras are some 90 cm from the center of the wing) 
			tmp[grepl(tmp$cameraID.x, pattern='cx'),]$offset<-1 	  # distance from transect = 60cm offset from inner camera
			tmp[grepl(tmp$cameraID.x, pattern='rx'),]$offset<-1.90    # distance from transect = 180cm offset from inner camera
			}
		
		if(grepl(x, pattern='20230604-T00_00Rx-T32_00Lx') & grepl(inner, pattern='rx')) {
			tmp[grepl(tmp$cameraID.x, pattern='rx'),]$offset<-0.1     # consider the distance of the inner camera from the transect (Diver was swimming at about 1m from the transect, and the external cameras are some 90 cm from the center of the wing) 
			tmp[grepl(tmp$cameraID.x, pattern='cx'),]$offset<-1 	  # distance from transect = 60cm offset from inner camera
			tmp[grepl(tmp$cameraID.x, pattern='lx'),]$offset<-1.90    # distance from transect = 180cm offset from inner camera
			}

		# shooting group 20230604-T28_00Rx-T60_00Lx
		if(grepl(x, pattern='20230604-T28_00Rx-T60_00Lx') & grepl(inner, pattern='lx')) {
			tmp[grepl(tmp$cameraID.x, pattern='lx'),]$offset<-0.1     # consider the distance of the inner camera from the transect (Diver was swimming at about 1m from the transect, and the external cameras are some 90 cm from the center of the wing) 
			tmp[grepl(tmp$cameraID.x, pattern='cl'),]$offset<-0.7 	  # distance from transect = 60cm offset from inner camera
			tmp[grepl(tmp$cameraID.x, pattern='cr'),]$offset<-1.30    # distance from transect = 120cm offset from inner camera
			tmp[grepl(tmp$cameraID.x, pattern='rx'),]$offset<-1.90    # distance from transect = 180cm offset from inner camera
			}
		
		if(grepl(x, pattern='20230604-T28_00Rx-T60_00Lx') & grepl(inner, pattern='rx')) {
			tmp[grepl(tmp$cameraID.x, pattern='rx'),]$offset<-0.1     # consider the distance of the inner camera from the transect (Diver was swimming at about 1m from the transect, and the external cameras are some 90 cm from the center of the wing) 
			tmp[grepl(tmp$cameraID.x, pattern='cr'),]$offset<-0.7 	  # distance from transect = 60cm offset from inner camera
			tmp[grepl(tmp$cameraID.x, pattern='cl'),]$offset<-1.30    # distance from transect = 120cm offset from inner camera
			tmp[grepl(tmp$cameraID.x, pattern='lx'),]$offset<-1.90    # distance from transect = 180cm offset from inner camera
			}


		# retrive leg direction from name
		legx<-unlist(strsplit(x,split=' '))[2]
		tr<-str_extract(legx, pattern='T[0-9]{2}')
		dir<-str_extract(legx, pattern='[0-9]{1,2}-[0-9]{1,2}')
		side<-str_extract(legx, pattern='Rx|Lx')
		# select correct X-bearing

		# # prep dataset for geosphere functions
		# tmp<-st_as_sf(tmp, coords=c('X','Y'),crs=4326) %>%
		# 				# st_set_crs() %>%
		# 				st_transform(crs=4326) %>%
		# 				mutate(X=st_coordinates(.)[,1],Y=st_coordinates(.)[,2])
		
		# find correct X-bearing and calculate offset
		Xtemp<-xlegs_hdg %>%
				filter(trs==tr, dir==leg | dir==rev_leg ) 
		Xhdg<-ifelse(side=='Rx',Xtemp$crRX, Xtemp$crLX ) 


		destPoint(p=cbind(tmp$X,tmp$Y), b=Xhdg, d=tmp$offset) %>% 
		as.data.frame() %>%
		bind_cols(tmp,.) %>%
		mutate(id=paste(step, cameraID.x)) %>%
...
# as.data.frame() %>% head()
# as.data.frame(.) %>% unique(.$camera_role)
...
xy_adj %>%
		...
		as.data.frame() %>%
		split(f=paste(.$leg,.$cameraID.x)) %>%	
		lapply(function(x){ 
			# browser()
			pt_1<-st_multipoint(cbind(x[x$step==1,'lon'],x[x$step==1,'lat']))
			pt_2<-st_multipoint(cbind(x[x$step==(max(x$step)-1),'lon'],x[x$step==(max(x$step)-1),'lat']))
...
xy_adj %>%
		...
		split(f=paste(.$leg,.$cameraID.x)) %>%	
		lapply(function(x){ #browser()
			pt_1<-st_multipoint(cbind(x[x$step==5,'lon'],x[x$step==5,'lat']))
			pt_2<-st_multipoint(cbind(x[x$step==(max(x$step)-5),'lon'],x[x$step==(max(x$step)-5),'lat']))
			plot(pt_1, bg='yellow', pch=21, add=T) 
			plot(pt_2, bg='purple', pch=21, add=T)
...
# calc mean depth per site to be used as altitude
head(depths)
str(depths)
# alti<-depths %>%
# 		filter(site=='MARINA-1') %>%
# 		with(.,mean(target_depth_adj)) %>%
# 		round(.,0)*-1

alti<-depths %>%
		group_by(site) %>%
		summarise(alti=round(mean(target_depth_adj),0)*-1, stdv=sd(target_depth_adj)) %>%
		filter(site=='MARINA-1') 
		# %>%
		# with(.,mean(target_depth_adj)) %>%
		# round(.,0)*-1
...
geo_pics<- xy_adj %>%
			rename('path'='file', 'longitude'='lon', 'latitude'='lat') %>%
			mutate(label=unlist(lapply(path, function(x) {
							res<-unlist(strsplit(x,split='/'))
							lres<-length(res)
							res[lres]
							})),
		    		altitude=alti$alti) %>%  # altitude is needed otherwise the data are not imported by Metashape
...
script_ver<-'imagery_georef-QD-MAR_1-offset.R'
...
Coordinates are first estimated for steps along the transect and then corrected to account for the lateral offset for each camera (i.e., as if the cameras were perpendicular to the trasnect) depending on their place on the rig.
Pictures in loops around posts are not georeferenced, except the first pic of the loop for each camera.

CRS= WGS84
')

shaRe::export_dictionary(description=txt, varTable= geo_dict, path='output/datasets/photogrammetry/georeferenced_images-QD-MAR_1-offset-dictionary.txt', formatTable='long')

write.csv(geo_pics,file='output/datasets/photogrammetry/georeferenced_images-QD-MAR_1-offset.csv', quote=F, row.names=F)
