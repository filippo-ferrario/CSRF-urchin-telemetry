...
line_0_30<-lapply(unique(leg_0_30$trs), function(x){ 
						#browser()
...
line_30_60<-lapply(unique(leg_30_60$trs), function(x){ 
						#browser()
...
# \/\/\/\/\\/\/\/\/\\/\/\/\/\\/\/\/\/\\/\/\/\/\\/\/\/\/\
# DEAL WITH EXCEPTION IN TRANSECT 52
# \/\/\/\/\\/\/\/\/\\/\/\/\/\\/\/\/\/\\/\/\/\/\\/\/\/\/\

# add split lines for transect 52 @ 40 and 43m towards 60
ratio43<-13/30
ratio40<-10/30

t52_30_43<-filter(legs, trs=='T52',leg=='30-60') %>% 
			st_linesubstring(from=0,to=ratio43) %>% # st_geometry(.) %>% plot(.,col=2, add=T)
			mutate(leg='30-43',rev_leg='43-30')
t52_40_60<-filter(legs, trs=='T52',leg=='30-60') %>% 
			st_linesubstring(from=ratio40,to=1) %>% #st_geometry(.) %>% plot(.,col=4, add=T)
			mutate(leg='40-60',rev_leg='30-43')	
...
# \/\/\/\/\\/\/\/\/\\/\/\/\/\\/\/\/\/\\/\/\/\/\\/\/\/\/\
# check EXCEPTION IN TRANSECT 52
# \/\/\/\/\\/\/\/\/\\/\/\/\/\\/\/\/\/\\/\/\/\/\\/\/\/\/\


picsList_coords_df %>%
		filter(grepl(.$leg,pattern='-43')) %>%
		split(f=.$leg) %>%	
		lapply(function(x){ #browser()
			pt_1<-st_multipoint(cbind(x[x$step==1,'X'],x[x$step==1,'Y']))
			pt_2<-st_multipoint(cbind(x[x$step==(max(x$step)),'X'],x[x$step==(max(x$step)),'Y']))
			plot(pt_1, bg='green', pch=24, add=T) 
			plot(pt_2, bg='green', pch=24, add=T)

			})

picsList_coords_df %>%
		filter(grepl(.$leg,pattern='40-')) %>%
		split(f=.$leg) %>%	
		lapply(function(x){ #browser()
			pt_1<-st_multipoint(cbind(x[x$step==5,'X'],x[x$step==5,'Y']))
			pt_2<-st_multipoint(cbind(x[x$step==(max(x$step)-5),'X'],x[x$step==(max(x$step)-5),'Y']))
			plot(pt_1, bg='blue', pch=22, add=T) 
			plot(pt_2, bg='blue', pch=22, add=T)
...
geo_pics<-picsList_coords_df %>%
	...
	mutate(label=unlist(lapply(path, function(x) {
		   ...
		   altitude=alti) %>%  # altitude is needed otherwise the data are not imported by Metashape
...
script_ver<-'imagery_georef-QD-MAR_2.R'
...
shaRe::export_dictionary(description=txt, varTable= geo_dict, path='output/datasets/photogrammetry/georeferenced_images-QD-MAR_2-dictionary.txt', formatTable='long')

write.csv(geo_pics,file='output/datasets/photogrammetry/georeferenced_images-QD-MAR_2.csv', quote=F, row.names=F)
