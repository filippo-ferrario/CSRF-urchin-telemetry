# ===============================================================================
# Name   	: Georeference Cameras for Marina 2
# Author 	: Filippo Ferrario
# Date   	: 2024-03-07 [dd-mm-yyyy]
# Version	: 
# URL		: 
...
# create tag for reverse leg  (2024-03-07: not so sure anymore that it actually needed since: 1) the Transect line is always the same 2) the direction is in the tag of the sequence, 3) the ordre of points in the end is just reversed in case of reverse leg....)
...
# --------------------------------------------------------------------------
# Add leg T52-30-43 and T52-40-60 to accomodate the specific case of the transect that has been interrupted during the filming in the leg 30-60 (right side thus).
# The filming has been interrupted at meter 43 and then resumed at meter 40 along the T52
plot(st_geometry(receivers), pch=16)
with(receivers, text(lon,lat-0.00002, label=xy_grid))
plot(st_geometry(t060_30), pch=16, col=4,add=T)
plot(st_geometry(t060_60), pch=16, col=5,add=T)
...
T52_30_43<-line_30_60 %>%
	filter(trs=='T52') %>%
	st_linesubstring(from=0, to=13/30) %>% 
	mutate(leg='30-43', rev_leg='43-30')
plot(st_geometry(T52_30_43),col=2, add=T, lwd=2)

T52_40_60<-line_30_60 %>%
	filter(trs=='T52') %>%
	st_linesubstring(from=10/30, to=1) %>%
	mutate(leg='40-60', rev_leg='60-43')
plot(st_geometry(T52_40_60),col=3, add=T, lwd=1)

legs<-bind_rows(legs, T52_30_43,T52_40_60)
as.data.frame(legs)
# --------------------------------------------------------------------------
...
# check the specific case of T52 leg 30-60
filter(pics, grepl(leg, pattern='_43|_40'))
...
picsList_coords<-lapply(names(picsList), function(x){
			...
			# define if it is a revers leg [as it is, the code already adapts to the legs 30-43 and 40-60 specific to this site! ]
			...
			# print(paste('===============ITERAZIONE ====',which(names(picsList)==x)) ) # crash after 63
			picXleg
			}
		) 
...
xy_adj<-lapply(names(leg_file_list), function(x){
		...
		if(grepl(inner, pattern='lx')) {
			...
			tmp[grepl(tmp$cameraID.x, pattern='cl'),]$offset<-0.70 	  # distance from transect = 60cm offset from inner camera
		...
		if(grepl(inner, pattern='rx')) {
			...
			tmp[grepl(tmp$cameraID.x, pattern='cr'),]$offset<-0.70 	  # distance from transect = 60cm offset from inner camera
...
xy_adj %>%
		filter(grepl(.$leg,pattern='-30-60|-43|40-')) %>%
		as.data.frame() %>%
		split(f=paste(.$leg,.$cameraID.x)) %>%	
		lapply(function(x){ #browser()
			pt_1<-st_multipoint(cbind(x[x$step==1,'lon'],x[x$step==1,'lat']))
			pt_2<-st_multipoint(cbind(x[x$step==(max(x$step)-0),'lon'],x[x$step==(max(x$step)-0),'lat']))
			plot(pt_1, bg='yellow', pch=21, add=T) 
			plot(pt_2, bg='purple', pch=21, add=T)
...
script_ver<-'imagery_georef-QD-MAR_2-offset.R'
...
shaRe::export_dictionary(description=txt, varTable= geo_dict, path='output/datasets/photogrammetry/georeferenced_images-QD-MAR_2-offset-dictionary.txt', formatTable='long')

write.csv(geo_pics,file='output/datasets/photogrammetry/georeferenced_images-QD-MAR_2-offset.csv', quote=F, row.names=F)
