# ===============================================================================
# Name   	: Georeference Cameras for Tugwell 1
# Author 	: Filippo Ferrario
# Date   	: 30-08-2023 [dd-mm-yyyy]
# Version	: 
# URL		: 
...
ref_pics<-read.csv('data/photogrammety/PR-Tug_1-pic2georef-main_camera.csv', na.strings = "")
pics<-read.csv('output/datasets/photogrammetry/PR-TUG_1-paired_synced.csv', na.strings = "")
...
tail(picsList_coords_df)
options( error=NULL ) 

options( error=recover ) 

# calclulate offset for support cameras 
# -----------------------------------------------------------------
...
xy_adj<-lapply(names(leg_file_list), function(x){
		...
		inner<- tmp %>%
				filter(camera_role=='inner camera') 
		...
		filter(!duplicated(id)) %>%		# here I get rid of pictures in loops, just keeping the first one.
		st_as_sf( coords=c('lon','lat')) %>%
		st_set_crs(4326)
...
xy_adj %>%
		...
		# filter(grepl(.$shooting_group,pattern='T00_00Lx-T28')) %>%
		# filter(grepl(.$shooting_group,pattern='T32_00Lx-T60')) %>% 
		# filter(grepl(.$shooting_group,pattern='T28_00Rx-T32')) %>%
		as.data.frame() %>%
...
geo_pics<-picsList_coords_df %>%
	...
	mutate(label=unlist(lapply(path, function(x) {
		   ...
		   altitude=-6) %>%  # altitude is needed otherwise the data are not imported by Metashape
...
script_ver<-'imagery_georef-PR-TUG_1.R'
...
shaRe::export_dictionary(description=txt, varTable= geo_dict, path='output/datasets/photogrammetry/georeferenced_images-PR-TUG_1-offset-dictionary.txt', formatTable='long')

write.csv(geo_pics,file='output/datasets/photogrammetry/georeferenced_images-PR-TUG_1-offset.csv', quote=F, row.names=F)
