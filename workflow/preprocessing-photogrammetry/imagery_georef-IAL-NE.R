# ===============================================================================
# Name   	: Georeference Cameras for IAL-SW
# Author 	: Filippo Ferrario
# Date   	: 03-10-2023 [dd-mm-yyyy]
# Version	: 
# URL		: 
# Aim 		:
# ===============================================================================
...
gps<-read.csv('output/datasets/photogrammetry/QC-targets_receivers_coordinates_depth.csv') 
ref_pics<-read.csv('data/photogrammetry/IAL-NE-pic2georef-main_camera.csv', na.strings = "") %>% as_tibble()
pics<-read.csv('output/datasets/photogrammetry/IAL-NE-paired_synced.csv', na.strings = "")
...
ialNE_gps<-gps %>%	
		filter(site=='IAL-NE', receiver_id!='Ref') %>%
		select(xy_grid, target_id, lat_target,lon_target) %>%
...
# Y=0
t030_0<-destPoint(p=st_coordinates(ialNE_gps[ialNE_gps$gcp=='T0-0',]), b=hc0[1], d=dists) %>%
		as.data.frame(.) %>%
		mutate(gcp=paste0('T',dists,'-0')) %>%
		st_as_sf(., coords=c('lon','lat')) %>%
		st_set_crs(4326) 
plot(st_geometry(t030_0), pch=16, col=3,add=T)

t3060_0<-destPoint(p=st_coordinates(ialNE_gps[ialNE_gps$gcp=='T30-0',]), b=hc0[2], d=dists) %>%
		as.data.frame(.) %>%
		mutate(gcp=paste0('T',dists+30,'-0')) %>%
		st_as_sf(., coords=c('lon','lat')) %>%
		st_set_crs(4326) 
plot(st_geometry(t3060_0), pch=16, col=4,add=T)

#  Y=30
dists030_30<-seq(3, as.numeric(st_distance(ialNE_gps[ialNE_gps$gcp=='T0-30',],ialNE_gps[ialNE_gps$gcp=='T30-30',]))-3, length.out=9)
t030_30<-destPoint(p=st_coordinates(ialNE_gps[ialNE_gps$gcp=='T0-30',]), b=hc30[1], d=dists030_30) %>%
		as.data.frame(.) %>%
		mutate(gcp=paste0('T',dists,'-30')) %>%
		st_as_sf(., coords=c('lon','lat')) %>%
		st_set_crs(4326) 
plot(st_geometry(t030_30), pch=16, col=5,add=T)

dists3060_30<-seq(3, as.numeric(st_distance(ialNE_gps[ialNE_gps$gcp=='T30-30',],ialNE_gps[ialNE_gps$gcp=='T60-30',]))-3, length.out=9)
t3060_30<-destPoint(p=st_coordinates(ialNE_gps[ialNE_gps$gcp=='T30-30',]), b=hc30[2], d=dists3060_30) %>%
		as.data.frame(.) %>%
		mutate(gcp=paste0('T',dists+30,'-30')) %>%
		st_as_sf(., coords=c('lon','lat')) %>%
		st_set_crs(4326) 
plot(st_geometry(t3060_30), pch=16, col=7,add=T)

# Y=60
dists030_60<-seq(3, as.numeric(st_distance(ialNE_gps[ialNE_gps$gcp=='T0-60',],ialNE_gps[ialNE_gps$gcp=='T30-60',]))-3, length.out=9)
t030_60<-destPoint(p=st_coordinates(ialNE_gps[ialNE_gps$gcp=='T0-60',]), b=hc60[1], d=dists030_60) %>%
		as.data.frame(.) %>%
		mutate(gcp=paste0('T',dists,'-60')) %>%
		st_as_sf(., coords=c('lon','lat')) %>%
		st_set_crs(4326) 
plot(st_geometry(t030_60), pch=16, col=6,add=T)

# dists3060_60<-seq(3, as.numeric(st_distance(ialNE_gps[ialNE_gps$gcp=='h',],ialNE_gps[ialNE_gps$gcp=='i',]))-3, length.out=9)
t3060_60<-destPoint(p=st_coordinates(ialNE_gps[ialNE_gps$gcp=='T30-60',]), b=hc60[2], d=dists) %>%
		as.data.frame(.) %>%
		mutate(gcp=paste0('T',dists+30,'-60')) %>%
		st_as_sf(., coords=c('lon','lat')) %>%
		st_set_crs(4326) 
...
geo_pics<-picsList_coords_df %>%
	...
	mutate(label=unlist(lapply(path, function(x) {
		   ...
		   altitude=-3) %>%  # altitude is needed otherwise the data are not imported by Metashape
...
script_ver<-'imagery_georef-IAL-NE.R'
...
shaRe::export_dictionary(description=txt, varTable= geo_dict, path='output/datasets/photogrammetry/georeferenced_images-IAL-NE-dictionary.txt')

write.csv(geo_pics,file='output/datasets/photogrammetry/georeferenced_images-IAL-NE.csv', quote=F, row.names=F)
