...
ref_pics<-read.csv('data/photogrammetry/IAL-SW-pic2georef-main_camera.csv', na.strings = "") %>% as_tibble()
gps<-read.csv('output/datasets/photogrammetry/QC-targets_receivers_coordinates_depth.csv') 
pics<-read.csv('output/datasets/photogrammetry/IAL-SW-paired_synced.csv', na.strings = "")
...
ial_gps<-gps %>%	
		filter(site=='IAL-SW', receiver_id!='Ref') %>%
...
# Y=0
t030_0<-destPoint(p=st_coordinates(ial_gps[ial_gps$gcp=='T0-0',]), b=hc0[1], d=dists) %>%
		as.data.frame(.) %>%
...
t3060_0<-destPoint(p=st_coordinates(ial_gps[ial_gps$gcp=='T30-0',]), b=hc0[2], d=dists) %>%
		as.data.frame(.) %>%
...
#  Y=30
t030_30<-destPoint(p=st_coordinates(ial_gps[ial_gps$gcp=='T0-30',]), b=hc30[1], d=dists) %>%
		as.data.frame(.) %>%
...
t3060_30<-destPoint(p=st_coordinates(ial_gps[ial_gps$gcp=='T30-30',]), b=hc30[2], d=dists) %>%
		as.data.frame(.) %>%
...
# Y=60
dists030_60<-seq(3, as.numeric(st_distance(ial_gps[ial_gps$gcp=='T0-60',],ial_gps[ial_gps$gcp=='T30-60',]))-3, length.out=9)
t030_60<-destPoint(p=st_coordinates(ial_gps[ial_gps$gcp=='T0-60',]), b=hc60[1], d=dists030_60) %>%
		as.data.frame(.) %>%
...
# dists3060_60<-seq(3, as.numeric(st_distance(ial_gps[ial_gps$gcp=='h',],ial_gps[ial_gps$gcp=='i',]))-3, length.out=9)
t3060_60<-destPoint(p=st_coordinates(ial_gps[ial_gps$gcp=='T30-60',]), b=hc60[2], d=dists) %>%
		as.data.frame(.) %>%
...
script_ver<-'imagery_georef-IAL-SW.R'
...
shaRe::export_dictionary(description=txt, varTable= geo_dict, path='output/datasets/photogrammetry/georeferenced_images-IAL-SW-dictionary.txt')

write.csv(geo_pics,file='output/datasets/photogrammetry/georeferenced_images-IAL-SW.csv', quote=F, row.names=F)
